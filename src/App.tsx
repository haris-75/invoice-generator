...
import { useBillingForm, useFormikForm, useSaveFormAPI } from '@hooks';
...
function App() {
  ...
  } = useBillingForm(formSubmitHandler);
  ...
  const {
    ...
    clearForm: clearInvoiceForm,
      ...
      invoiceFormInitialValues, formSubmitHandler , InvoiceFormSchema
    );
  ...
  const { createInvoice, data, loading, error } = useSaveFormAPI();
      ...
      variables: {
        input: {
          createInvoiceAttributes: {
            invoiceDate: invoiceValues?.date,
            paymentTerms: invoiceValues?.terms?.toUpperCase()?.split(' ')?.join('_'),
            projectDescription: invoiceValues?.description,
            billingToAttributes: {
            ...
            billingFromAttributes: {
                ...
                companyEmail: companyValues?.email,
            ...
            itemAttributes: itemArray?.map(({name, quantity, price})=>(            {
              name,
